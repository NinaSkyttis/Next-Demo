  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="grid grid-cols-5 gap-4">
        ...
        <p className={`text-center capitalized textGradient col-span-3 whitespace-nowrap ` + fugaz.className}> {selectedMonth}, {selectedYear} </p>
        <button onClick={() => {
          handleIncrementMonth(+1)
        }}
          ...
          return (
            <div key={rowIndex} className="grid grid-cols-7 gap-1">
              {dayList.map((dayOfWeek, dayOfWeekIndex) => {
                ...
                return (
                  <div style={{background: color}} className={'text-xs sm:text-sm border border-solid p-2 flex items-center gap-2 justifybetween rounded-lg ' + (isToday ? 'border-indigo-400 ' : ' border-indigo-100 ') + (color === 'white' ? 'text-indigo-400' : 'text-white')} key={dayOfWeekIndex}>
                    <p>{dayIndex}</p>
                  </div>
                )
              })}
